    ...
    public class DataSetHelper
    {
            ...
            */
            ...
            else
            {
                ...
                foreach (FieldInfo Field in GroupByFieldInfo)
                {
                    ...
                    if ((dc == null) && (Field.Aggregate.StartsWith("count")))
                    {
                        dc = new DataColumn();
                                        ...
                                        string RowFilter, string GroupBy)
        {
            ...
            */
            ...
            DataRow LastSourceRow = null, DestRow = null; bool SameRow; int RowCount = 0; int RowCountDistinct1 = 1;
            foreach (DataRow SourceRow in Rows)
            {
                ...
                if (!SameRow)
                {
                    ...
                    RowCount = 0;
                    RowCountDistinct1 = 1; //just use for one Field
                }
                RowCount += 1;
                foreach (FieldInfo Field in GroupByFieldInfo)
                {
                    switch (Field.Aggregate)    //this test is case-sensitive
                    {
                        ...
                        case "count":
                            ...
                            break;
                        case "countdistinct":
                            //if (LastSourceRow[Field.FieldName] == SourceRow[Field.FieldName])
                            //{
                            //    int xx = 1;
                            //}
                            if (LastSourceRow != null)
                            {
                                if (LastSourceRow[Field.FieldName].ToString().CompareTo(SourceRow[Field.FieldName].ToString()) != 0)
                                {
                                    if (SameRow) RowCountDistinct1++;
                                }
                            }
                            DestRow[Field.FieldAlias] = RowCountDistinct1;
                            break;
                        case "sum":
                                         ...
                                         string RowFilter, string GroupBy)
        {
             ...
             */
            System.Data.DataTable dt = CreateGroupByTable(TableName, SourceTable, FieldList);
