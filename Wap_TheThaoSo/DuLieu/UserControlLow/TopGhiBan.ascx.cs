...
{
    public partial class TopGhiBan : BaseControl
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (competitionId > 0)
            {
                BxhLink = UrlProcess.GetTopBxhLowUrl(competitionId) + "&seasonId=" + seasonId;
                TopGhiBanLink = UrlProcess.GetTopGhiBanLowUrl(competitionId) + "&seasonId=" + seasonId;
                CalendarLink = UrlProcess.GetLichThiDauLow(competitionId) + "&seasonId=" + seasonId;

                DataSet ds = _duLieuController.WapTheThaoSoGetTopGoalByCompetitionAndSeason(competitionId, seasonId);
                if (ds != null)
                {
                    ...
                    if (competitionId == 8) // Anh
                    {
                        ...
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=8F100B80-CDC2-4983-A20A-95E6D05392BA";
                    }
                    else if (competitionId == 7) // Tay Ban Nha
                    {
                        divTranCauSangNhat.Visible = true;
                        litName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=D49D7438-863E-432F-BB55-37B5CE7916E0";
                    }
                    else if (competitionId == 13) // Italia
                    {
                        divTranCauSangNhat.Visible = true;
                        litName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=D73803EA-CC9F-42A3-B0BC-1CC80EFE1E74";
                    }
                    else if (competitionId == 9) // Duc
                    {
                        divTranCauSangNhat.Visible = true;
                        litName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=2D08173B-7221-4C73-A783-4505F1C5A63B";
                    }
                    else if (competitionId == 16) // Phap
                    {
                        divTranCauSangNhat.Visible = true;
                        litName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=4519653E-F1A6-4DA7-84B8-56E352C58C1B";
                    }
                    else if (competitionId == 10) // C1
                    {
                        divTranCauSangNhat.Visible = true;
                        litName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=F6F9038B-EE68-4E50-BF50-91C8308B3044";
                    }
                    else if (competitionId == 18) // Europa league
                    {
                        divTranCauSangNhat.Visible = true;
                        litName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                        lnkBamDay.NavigateUrl = "/DuLieu/DefaultLow.aspx?display=tuvan&w=" + Lang + "&id=B2172860-8127-4A94-9CCE-66DA9B04B62B";
                    }
                    ...
                    {
                        rptTopGoals.DataSource = ds.Tables[1];
                        rptTopGoals.DataBind();
                    }
                }
            }
        }
