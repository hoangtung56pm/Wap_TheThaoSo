...
namespace Wap_TheThaoSo.DuLieu.UserControl.Pagging
{
    ...
    public partial class MatchPagging : BaseControl
    {
        //trang hien tai
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //{
                    ...
                    int cpage = ConvertUtility.ToInt32(Request.QueryString["mpage"]);
                    if (cpage > 0)
                    {
                        curpage = cpage;
                    }
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (totalrecord > 0 && pagesize > 0 && numberpage > 0)
            {
                ...
                if (1 < curpage)
                {
                    lnkFirst.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/DuLieu/", "") + queryparam + (1).ToString();
                    lnkPrev.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/DuLieu/", "") + queryparam + (curpage - 1).ToString();
                }
                if (curpage < totalpage)
                {
                    lnkLast.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/DuLieu/", "") + queryparam + totalpage.ToString();
                    lnkNext.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/DuLieu/", "") + queryparam + (curpage + 1).ToString();
                }
            }
            else
            {
        ...
        public void rptPage_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            string url = Request.AppRelativeCurrentExecutionFilePath.Replace("~/DuLieu/", "") + queryparam + e.Item.DataItem;
