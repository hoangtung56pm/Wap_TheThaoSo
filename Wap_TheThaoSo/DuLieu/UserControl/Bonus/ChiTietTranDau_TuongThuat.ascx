<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ChiTietTranDau_TuongThuat.ascx.cs" Inherits="Wap_TheThaoSo.DuLieu.UserControl.Bonus.ChiTietTranDau_TuongThuat" %>
            ...
            <div class="item">
                ...
                <div class="chitietdoibong">
                    <asp:Repeater runat="server" ID="rptTeamInfo" EnableViewState="false">
                        <ItemTemplate>
                            ...
                            <div class="bgred">
                                ...
                                <b class="bold color-white">Giải đấu:</b>&nbsp;<%# Eval("CompetitionName") %><%--<%# !string.IsNullOrEmpty(Eval("CompetitionName").ToString()) ? Eval("CompetitionName") : Eval("CompetitionName")%>--%><div class="clear2px">
                                </div>
                                <b class="bold color-white">Ngày:</b>&nbsp;<%# Eval("match_time") %><div class="clear2px">
                                </div>
                                <b class="bold color-white">Giờ đá:</b>&nbsp;<%# Eval("match_day") %><%--<%# AppEnv.ConvertTime(Eval("TimeHour").ToString()) %>:<%# AppEnv.ConvertTime(Eval("TimeMinute").ToString()) %>--%><div class="clear2px">
                                </div>
                                <b class="bold color-white">SVĐ:</b>&nbsp;<%# Eval("Stadium") %></div>
                        </ItemTemplate>
                    </asp:Repeater>
                    <div class="clear10px">
                    </div>
                    <div class="phongdo">
                            ...
                            <ItemTemplate>
                                ...
                                <asp:Panel ID="pnlVideo" runat="server" Visible="false">
                                    ...
                                    <div class="floatleft height19px background-xam-b8b8b8 width32percent text-align-center margin-left0px">
                                        ...
                                        <a href="<%= GetVideoLink() %>" class="color-xam737373 lineheight padding-left5px padding-right5px">Video</a>
                                    </div>
                        ...
                        <asp:Panel ID="pnlTuongThuat" runat="server" Visible="true">
                                ...
                                <ItemTemplate>
                                    <div class="item font12px">
                                        <div class="width50px floatleft padding-left5px lineheight">
                                            <%# Eval("game_minute") %>'<asp:Literal runat="server" EnableViewState="false" ID="litExtraTime"></asp:Literal>
                                            <asp:Literal ID="litImage" runat="server"></asp:Literal>
                                        </div>
                                        <div class="floatleft width80percent lineheight">
                                            <%--<%# AppEnv.Translate(Eval("Code").ToString(),Eval("Content").ToString()) %>--%>
                                            <%# AppEnv.TranslateApi(Eval("type").ToString(),Eval("matchname").ToString(),Eval("game_minute").ToString()) %>
                                        </div>
                                    </div>
                                </ItemTemplate>
